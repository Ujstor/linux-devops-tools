...
| 65 | **ai** | Claude Code and opencode as **base** installs via their vendor installers; crush opt-in |
...
| V5 | No AI-agent module; the agent CLI installed through the JavaScript package manager. | **`modules/65-ai.sh`, and the agent installed by its own native installer as a base install** in the default profile, install-if-absent, never updated by this repository. | Verified on the reference machine: the package manager had nothing, the native installer had five self-updating versions. The other agents became an opt-in profile; opencode later joined Claude Code as a base install on the same install-if-absent terms. |
