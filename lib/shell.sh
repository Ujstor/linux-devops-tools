...
# the literal string `devops-env-config` within its first 5 lines. bashrc_dropin
...
# "# >>> devops-env-config >>>". One definition, one detector, one uninstaller.
...
#   Warns when the payload lacks the `devops-env-config` marker, because the pruner
...
bashrc_dropin_remove() {
  ...
  [ -f "$f" ] || return 0
  if ! head -n5 "$f" | grep -q "$DEVENV_TAG"; then
...
bashrc_ensure_local() {
  ...
  ensure_dir "$DEVENV_DROPIN_DIR" || return 1
  printf '%s\n' \
    '# ~/.bashrc.d/90-local.sh — yours. devops-env-config creates this file once' \
...
comp_is_ours() {
  [ -f "$1" ] || return 1
  head -n1 -- "$1" | grep -Fq -- "$DEVENV_TAG generated"
}
...
comp_cache() {
  ...
  if [ -f "$out" ] && ! comp_is_ours "$out"; then
    log_warn "$out exists and was not generated by devops-env-config — leaving it alone"
    return 0
  fi
...
comp_prune() {
  ...
  log_info "cleared the devops-env-config completion cache in $dir"
  return 0
}
